<template>
    <basic-container>
        <div class="deyatech-container pull-auto">
            ...
            <el-table :data="branchSchoolList" v-loading.body="listLoading" stripe border highlight-current-row
            ...
            <el-dialog :title="titleMap[dialogTitle]" :visible.sync="dialogVisible"
                       :close-on-click-modal="closeOnClickModal" @close="closeBranchSchoolDialog">
                <el-form ref="branchSchoolDialogForm" class="deyatech-form" :model="branchSchool" label-position="right"
                         label-width="80px" :rules="branchSchoolRules">
                    <el-row :gutter="20" :span="24">
                        <el-col :span="12">
                            <el-form-item label="名称" prop="name">
                                <el-input v-model.trim="branchSchool.name"></el-input>
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row :gutter="20" :span="24">
                        <el-col :span="24">
                            <el-form-item :label="$t('table.remark')" prop="remark">
                                <el-input type="textarea" v-model.trim="branchSchool.remark" :rows="3"/>
                            </el-form-item>
                        </el-col>
                    </el-row>
                </el-form>
                <span slot="footer" class="dialog-footer">
                    ...
                    <el-button :size="btnSize" @click="closeBranchSchoolDialog">{{$t('table.cancel')}}</el-button>
                </span>
            </el-dialog>


            <el-dialog :title="titleMap['associateUser']" :visible.sync="dialogUsersVisible"
                       :close-on-click-modal="closeOnClickModal" @close="closeBranchSchoolUserDialog">
                <div v-loading="dialogUsersFormLoading">
                    ...
                    <div>
                        <el-table ref="branchSchoolUserTable" :data="userList" border @select="selectRowUser"
                                  @select-all="selectAllUser" @selection-change="handleSelectionChangeBranchSchoolUser">
                        ...
                        <el-pagination class="deyatech-pagination pull-right" background
                                       ...
                                       @size-change="handleSizeChangeBranchSchoolUser" @current-change="handleCurrentChangeBranchSchoolUser">
                        </el-pagination>
                    </div>
                </div>
                <div slot="footer" class="dialog-footer">
                    <el-button type="primary" :size="btnSize" @click="doSaveBranchSchoolUser"
                               :loading="submitLoading">{{$t('table.confirm')}}</el-button>
                    <el-button :size="btnSize" @click="closeBranchSchoolUserDialog">{{$t('table.cancel')}}</el-button>
                </div>
            </el-dialog>
        </div>
    </basic-container>
...
<script>
    ...
    import {
        getBranchSchoolList,
        createOrUpdateBranchSchool,
        delBranchSchools,
        isExistName
    } from '@/api/reissue/branchSchool';
    import {
        getAllBranchSchoolUser,
        getBranchSchoolUserList,
        saveBranchSchoolUsers
    ...
    export default {
        ...
        computed: {
            ...
            btnEnable() {
                return {
                    create: this.permission.branchSchool_create,
                    update: this.permission.branchSchool_update,
                    delete: this.permission.branchSchool_delete,
                    users: this.permission.branchSchool_users,
                };
            }
        },
        created(){
        ...
        methods: {
            ...
            reloadList(){
                ...
                this.branchSchoolList = undefined;
                this.total = undefined;
                getBranchSchoolList(this.listQuery).then(response => {
                    this.listLoading = false;
                    this.branchSchoolList = response.data.records;
            ...
            btnUpdate(row){
                this.resetBranchSchool();
                if (row.id) {
                    this.branchSchool = deepClone(row);
                } else {
                    this.branchSchool = deepClone(this.selectedRows[0]);
                }
            ...
            doCreate(){
                this.$refs['branchSchoolDialogForm'].validate(valid => {
                    if(valid) {
                        this.submitLoading = true;
                        createOrUpdateBranchSchool(this.branchSchool).then(() => {
                            this.resetBranchSchoolDialogAndList();
            ...
            resetBranchSchoolDialogAndList(){
                this.closeBranchSchoolDialog();
                this.submitLoading = false;
                this.reloadList();
            },
            closeBranchSchoolDialog() {
                this.dialogVisible = false;
                this.resetBranchSchool();
                this.$refs['branchSchoolDialogForm'].resetFields();
            },
            ...
            btnUsers(row) {
                ...
                this.loadBranchSchoolUser(row.id).then(res => {
                    if (res && res.length > 0) {
                        for (let branchSchoolUser of res) {
                            this.selectAllUserId.push(branchSchoolUser.userId)
                        }
            ...
            loadBranchSchoolUser(schoolId) {
                let query = {schoolId}
                return new Promise((resolve, reject) => {
                    getAllBranchSchoolUser(query).then(response => {
            ...
            handleShowRelated(checked) {
                if (checked) {
                    this.userListQuery.schoolId = this.currentRow.id;
                } else {
                    this.userListQuery.schoolId = undefined;
                }
                this.reloadUserList();
            },
            reloadUserList() {
                this.handleCurrentChangeBranchSchoolUser(1)
            },
            ...
            handleSelectionChangeBranchSchoolUser(rows) {
                this.selectedRowsUser = rows;
            },
            handleSizeChangeBranchSchoolUser(val) {
                this.userListQuery.size = val;
                this.loadUserList().then(() => {
                    this.checkRelatedUserRows();
                });
            },
            handleCurrentChangeBranchSchoolUser(val) {
            ...
            loadUserList() {
                return new Promise((resolve, reject) => {
                    this.dialogUsersFormLoading = true;
                    getBranchSchoolUserList(this.userListQuery).then(response => {
                    ...
                    if (this.selectAllUserId && this.selectAllUserId.length > 0) {
                        for (let row of this.userList) {
                            if (this.selectAllUserId.includes(row.userId)) {
                                this.$refs['branchSchoolUserTable'].toggleRowSelection(row, true)
                            }
                        }
                    }
                });
            },
            closeBranchSchoolUserDialog() {
            ...
            doSaveBranchSchoolUser() {
                this.submitLoading = true;
                saveBranchSchoolUsers(this.currentRow.id, this.selectAllUserId).then(() => {
                    this.closeBranchSchoolUserDialog();
